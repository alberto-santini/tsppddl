...
std::vector<std::vector<int>> MipSolver::solve(const bool include_mtz, const bool use_lagrange_cycles, const bool use_lagrange_precedence, const bool use_valid_y_ineq, const bool k_opt) const {
    ...
    std::cerr << "IloNumArrays of dimensions => x: " << x.getSize() << ", y: " << y.getSize() << ", t: " << t.getSize() << std::endl;
    for(int i = 0; i <= 2 * n + 1; i++) {
        for(int j = 0; j <= 2 * n + 1; j++) {
            if(c[i][j] >= 0) {
                std::cerr << "Checking if x[" << col_index << "] > 0" << std::endl;
                if(x[col_index] > 0) {
                ...
                std::cerr << "Checking if y[" << col_index << "] > 0" << std::endl;
                if(y[col_index] > 0) {
         ...
         if(include_mtz) {
             std::cerr << "Printing t[" << i << "]" << std::endl;
