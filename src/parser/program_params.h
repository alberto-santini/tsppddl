...
struct program_params  {
    ...
    program_params() {}
    program_params(k_opt_params ko, subgradient_params sg, branch_and_cut_params bc, tabu_search_params ts, unsigned int cplex_threads, unsigned int cplex_timeout) : ko{std::move(ko)}, sg{std::move(sg)}, bc{std::move(bc)}, ts{std::move(ts)}, cplex_threads{cplex_threads}, cplex_timeout{cplex_timeout} {}
};
